...
function Asteroids() {
	...
	function hideElement(element) {
		if (!element || element.__asteroidsHidden) return;
		element.__asteroidsHidden = true;
		element.__asteroidsPrevDisplay = element.style.display;
		element.style.display = "none";
		that.hiddenElements.push(element);
	};

	function restoreHiddenElements() {
		for (let i = 0; i < that.hiddenElements.length; i++) {
			const element = that.hiddenElements[i];
			if (!element || !element.__asteroidsHidden) continue;
			element.style.display = typeof element.__asteroidsPrevDisplay === "string"
				? element.__asteroidsPrevDisplay
				: "";
			if (element.classList) element.classList.remove("ASTEROIDSYEAHENEMY");
			delete element.__asteroidsPrevDisplay;
			delete element.__asteroidsHidden;
		}
		that.hiddenElements = [];
	};
	...
	this.update = function() {
		...
		if (this.dying.length) {
			for (let i = this.dying.length - 1; i >= 0; i--) {
				const dyingElement = this.dying[i];
				try {
					if (dyingElement && dyingElement.parentNode) {
						hideElement(dyingElement);
